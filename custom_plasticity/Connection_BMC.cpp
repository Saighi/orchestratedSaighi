...
void Connection_BMC::init(AurynFloat eta, AurynFloat kappa, AurynFloat maxweight)
{
	set_name("Connection_BMC");
...
void Connection_BMC::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void Connection_BMC::init_shortcuts() 
{
...
Connection_BMC::Connection_BMC(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{

}

Connection_BMC::Connection_BMC(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat eta, 
		AurynFloat kappa, 
...
inline AurynWeight Connection_BMC::dw_pre(const NeuronID post, const AurynWeight * w)
{

	AurynDouble dw = A2_minus*(tr_post->get(post))*pow((*w)/wmax,8);
	
	return dw;
}

inline AurynWeight Connection_BMC::dw_post(const NeuronID pre, const NeuronID post, const AurynWeight * w)
{
	AurynDouble p = tr_post->get(post);

	AurynDouble dw = A3_plus*tr_pre->get(pre)*pow((wmax-*w)/wmax,8);

	return dw;

}

void Connection_BMC::propagate_forward()
{
	// loop over spikes
...
void Connection_BMC::propagate_backward()
{
...
void Connection_BMC::push_attributes()
{
...
void Connection_BMC::set_tau_f(AurynFloat tauf) {
	tau_f = tauf;
}


void Connection_BMC::set_tau_d(AurynFloat taud) {
	tau_d = taud;
}

void Connection_BMC::set_ujump(AurynFloat r) {
	Ujump = r;
}

void Connection_BMC::set_urest(AurynFloat r) {
	Urest = r;
}

bool Connection_BMC::write_to_file(string filename)
{
...
bool Connection_BMC::load_from_file(string filename)
{
...
void Connection_BMC::load_fragile_matrix(string filename)
{
