#!/bin/sh
{
	...
	OUTDIR="/mnt/data1/data_paul/long_adaptation/sim_long_adaptation_delta0.04"
	...
	make -C $DIR -j8 sim_rc_p10c_P_dicted_long_adapt && mpirun -n $NP $DIR/sim_rc_p10c_P_dicted_long_adapt \
		--dir $OUTDIR \
		--prefix rf1 --size $SIZE --save \
		--wie 0.08 --wee 0.1 --wext 2 --wei 0.72 --wii 0.08 \
		--simtime $TIME --tauf $TAUF --taud $TAUD \
		--intsparse 0.05 \
		--extsparse 0.2 \
		--off 2.0 --on 1.0 \
		--beta $BETA --eta $ETA --scale $SCALE --weight_a $WEIGHTA --alpha $ALPHA --delta 0.04 \
