...
int main(int ac, char* av[]) 
{
	...
	// sprintf(strbuf, "%s_%s", recfile.c_str(),"ee");
	// con_stim_e->load_fragile_matrix(recfile);
	sprintf(strbuf, "%s_n%s.wmat", recfile.c_str(),"ee");
	con_ee->load_fragile_matrix(string(strbuf));
	sprintf(strbuf, "%s_n%s.wmat", recfile.c_str(),"ie");
	con_i2e->load_from_complete_file(string(strbuf));
	sprintf(strbuf, "%s_n%s.wmat", recfile.c_str(),"ii");
	con_i2i2->load_from_complete_file(string(strbuf));
	sprintf(strbuf, "%s_n%s.wmat", recfile.c_str(),"ei");
	con_ei2->load_from_complete_file(string(strbuf));
