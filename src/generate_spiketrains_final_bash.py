...
parser.add_argument('-bashsize',type=int,required=False)
...
for s in range(nb_segment):
    ...
    if pattern:
        ...
        choices_patterns = np.random.randint(0,nb_pattern,int(len(pattern_times)/bash_size))
        choices_patterns = np.repeat(choices_patterns,bash_size)
        choices_patterns = np.append(choices_patterns,np.random.randint(0,nb_pattern, len(pattern_times)-len(choices_patterns)))
