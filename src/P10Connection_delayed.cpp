...
#include "P10Connection_delayed.h"

using namespace auryn;
using namespace std;

void P10Connection_delayed::init(AurynFloat eta, AurynFloat kappa, AurynFloat maxweight)
{
	set_name("P10Connection_delayed");
...
void P10Connection_delayed::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void P10Connection_delayed::init_shortcuts() 
{
...
void P10Connection_delayed::free()
{
...
P10Connection_delayed::P10Connection_delayed(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{

}

P10Connection_delayed::P10Connection_delayed(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat eta, 
		AurynFloat kappa, 
...
P10Connection_delayed::~P10Connection_delayed()
{
...
inline AurynWeight P10Connection_delayed::dw_pre(const NeuronID post, const AurynWeight * w)
{
	AurynDouble dw = A2_minus*(tr_post->get(post))-delta;
	return dw;
}

inline AurynWeight P10Connection_delayed::dw_post(const NeuronID pre, const NeuronID post, const AurynWeight * w, const AurynWeight w0)
{
...
void P10Connection_delayed::propagate_forward()
{
	// loop over spikes
...
void P10Connection_delayed::propagate_backward()
{
...
void P10Connection_delayed::propagate()
{
...
void P10Connection_delayed::evolve()
{
	if ( src->evolve_locally() ) { 
		// dynamics of x
...
void P10Connection_delayed::consolidate() {
	for (AurynLong i = 0 ; i < w_solid_matrix->get_nonzero() ; ++i ) {
		AurynWeight * fragile = w->get_data_begin()+i;
		AurynWeight * solid   = w_solid_matrix->get_data_begin()+i;
		*solid = *fragile;
...
void P10Connection_delayed::push_attributes()
{
...
void P10Connection_delayed::set_tau_f(AurynFloat tauf) {
	tau_f = tauf;
}

void P10Connection_delayed::set_tau_cons(AurynFloat taucons) {
...
void P10Connection_delayed::set_tau_d(AurynFloat taud) {
	tau_d = taud;
}

void P10Connection_delayed::set_ujump(AurynFloat r) {
	Ujump = r;
}

void P10Connection_delayed::set_urest(AurynFloat r) {
	Urest = r;
}

void P10Connection_delayed::set_weight_a(AurynFloat w0) {
	weight_a = w0;
	weight_b = (weight_a+weight_c)/2;
}

void P10Connection_delayed::set_weight_c(AurynFloat w2) {
	weight_c = w2;
	weight_b = (weight_a+weight_c)/2;
}

void P10Connection_delayed::set_beta(AurynFloat b)
{
	beta = b;
	beta_fudge = beta; 
...
bool P10Connection_delayed::write_to_file(string filename)
{
...
bool P10Connection_delayed::load_from_file(string filename)
{
...
void P10Connection_delayed::load_fragile_matrix(string filename)
{
...
void P10Connection_delayed::randomize_consolidation_variables(AurynFloat mean, AurynFloat std)
{
