...
int main(int ac, char* av[]) 
{
  ...
  string prefile = ""; // preload patters file
	...
	PoissonFileInputGroupRandom *stimgroup;

	if (!nopattern) {
	sprintf(strbuf, "%s/%s", dir.c_str(), "pattern");
	string pat = string(strbuf);
	sprintf(strbuf, "%s/%s", dir.c_str(), "pattern_time");
	string times_pat = string(strbuf);
	stimgroup = new PoissonFileInputGroupRandom(size, pat, times_pat, bgrate,
												nb_pattern);
	} else {
	stimgroup = new PoissonFileInputGroupRandom(size, bgrate);
	}
