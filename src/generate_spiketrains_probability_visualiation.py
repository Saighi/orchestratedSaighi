...
def outside_pattern(spiketrain,new_spiketrain,pattern_times):
    ...
    for i in range(len(spiketrain)):
        ...
        if not spiketrain[i]<pattern_times[actual_pattern]+time_pattern :
...
def simulate_pattern_neuron(n,neuron_pattern_pools,neuron_pattern_sizes):
    ...
    new_spiketrain = np.empty( (int(((rate*time_sim)/nb_segment)*10),2 ) ,dtype=np.float32)
...
if oscillation:
    ...
    X, Y = np.meshgrid(x, y)
    Z=f(X,Y)
    ax.scatter(fill_data[:,0],fill_data[:,1],c=["red" if i == 1 else "blue" for i in fill_data[:,2]],s=5,zorder=3)
    ax.contour(X,Y,Z,100,cmap='Greys')
    plt.show()
#%%
# plt.scatter(fill_data[:,0],fill_data[:,1],c=["red" if i == 1 else "blue" for i in fill_data[:,2]],s=5)
# fig = px.scatter(x=fill_data[:,0], y=fill_data[:,1],color=["red" if i == 1 else "blue" for i in fill_data[:,2]])
# fig.show()

#%%
spk = SpikeTrain(fill_data[:,0]*pq.s,np.max(fill_data[:,0])*pq.s)
rte = instantaneous_rate(spk,0.1*pq.s,kernels.GaussianKernel(sigma=100 * pq.ms))
