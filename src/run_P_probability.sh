#!/bin/sh
{
	...
	OUTDIR="/mnt/data1/data_paul/stronger_inhibition/sim_test_probability"
	...
	make -C $DIR -j8 sim_rc_p10c_P_dicted && mpirun -n $NP $DIR/sim_rc_p10c_P_dicted \
		...
		--wie 0.4 --wee 0.3 --wext 0.4 --wei 0.26 --wii 0.08 \
		--simtime $TIME --tauf $TAUF --taud $TAUD \
		--intsparse 0.05 \
		--extsparse 0.2 \
		--off 2.0 --on 1.0 \
		--beta $BETA --eta $ETA --scale $SCALE --weight_a $WEIGHTA --alpha 4 --delta 0.02 \
