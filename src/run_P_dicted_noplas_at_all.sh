...
OUTDIR="/mnt/data1/data_paul/sim_noplas_at_all_not_directed"
SPIKETRAINS_FILE="spiketrains"
mkdir -p $OUTDIR

#python generate_spiketrains.py 10 0 20 5 $TIME 3 $SIZE $NB_SEGMENT $OUTDIR false
#python generate_spiketrains_final.py -rate 10 -timesim $TIME -nbneurons $SIZE -nbsegment $NB_SEGMENT -outdir $OUTDIR

make -C $DIR -j8 sim_rc_p10c_P_dicted_noplas_at_all && mpirun -n $NP $DIR/sim_rc_p10c_P_dicted_noplas_at_all \
	--dir $OUTDIR \
