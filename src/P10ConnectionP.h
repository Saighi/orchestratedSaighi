...
#ifndef P10CONNECTIONP_H_
#define P10CONNECTIONP_H_

#include "auryn.h"

#define P10DECAYTERM_H 4.0
#define P10CONNECTIONP_EULERUPGRADE_STEP 0.99

namespace auryn {

class P10ConnectionP : public DuplexConnection
{
...
public:
	...
	P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter=GLUT);

	P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, 
			const char * filename, 
			AurynFloat eta=1, 
	...
	P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, 
			AurynWeight weight,std::string filename, AurynFloat sparseness=0.05, 
			AurynFloat eta=1, 
			AurynFloat kappa=3., AurynFloat maxweight=100. , 
			TransmitterType transmitter=GLUT,
			string name = "P10ConnectionP");

	virtual ~P10ConnectionP();
