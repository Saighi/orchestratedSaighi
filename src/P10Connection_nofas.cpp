...
#include "P10Connection_nofas.h"

using namespace auryn;
using namespace std;

void P10Connection_nofas::init(AurynFloat eta, AurynFloat kappa, AurynFloat maxweight)
{
	set_name("P10Connection_nofas");
...
void P10Connection_nofas::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void P10Connection_nofas::init_shortcuts() 
{
...
P10Connection_nofas::P10Connection_nofas(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{

}

P10Connection_nofas::P10Connection_nofas(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat eta, 
		AurynFloat kappa, 
...
P10Connection_nofas::~P10Connection_nofas()
{
...
inline AurynWeight P10Connection_nofas::dw_pre(const NeuronID post, const AurynWeight * w)
{
	AurynDouble dw = A2_minus*(tr_post->get(post))-delta;
	return dw;
}

inline AurynWeight P10Connection_nofas::dw_post(const NeuronID pre, const NeuronID post, const AurynWeight * w, const AurynWeight w0)
{
...
void P10Connection_nofas::propagate_forward()
{
	// loop over spikes
...
void P10Connection_nofas::propagate_backward()
{
...
void P10Connection_nofas::propagate()
{
...
void P10Connection_nofas::evolve()
{
	if ( src->evolve_locally() ) { 
		// dynamics of x
...
void P10Connection_nofas::consolidate() {
	for (AurynLong i = 0 ; i < w_solid_matrix->get_nonzero() ; ++i ) {
		AurynWeight * fragile = w->get_data_begin()+i;
		AurynWeight * solid   = w_solid_matrix->get_data_begin()+i;
		*solid = *fragile;
...
void P10Connection_nofas::push_attributes()
{
	...
	for (SpikeContainer::const_iterator spike = spikes->begin() ;
		...
		auryn_vector_float_set( state_x, spk, x-Urest*x );
...
void P10Connection_nofas::set_tau_f(AurynFloat tauf) {
	tau_f = tauf;
}

void P10Connection_nofas::set_tau_cons(AurynFloat taucons) {
...
void P10Connection_nofas::set_tau_d(AurynFloat taud) {
	tau_d = taud;
}

void P10Connection_nofas::set_ujump(AurynFloat r) {
	Ujump = r;
}

void P10Connection_nofas::set_urest(AurynFloat r) {
	Urest = r;
}

void P10Connection_nofas::set_weight_a(AurynFloat w0) {
	weight_a = w0;
	weight_b = (weight_a+weight_c)/2;
}

void P10Connection_nofas::set_weight_c(AurynFloat w2) {
	weight_c = w2;
	weight_b = (weight_a+weight_c)/2;
}

void P10Connection_nofas::set_beta(AurynFloat b)
{
	beta = b;
	beta_fudge = beta; 
...
bool P10Connection_nofas::write_to_file(string filename)
{
...
bool P10Connection_nofas::load_from_file(string filename)
{
...
void P10Connection_nofas::load_fragile_matrix(string filename)
{
...
void P10Connection_nofas::randomize_consolidation_variables(AurynFloat mean, AurynFloat std)
{
