...
void P10ConnectionP::init(AurynFloat eta, AurynFloat kappa, AurynFloat maxweight)
{
	set_name("P10ConnectionP");
...
void P10ConnectionP::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void P10ConnectionP::init_shortcuts() 
{
...
P10ConnectionP::P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{

}

P10ConnectionP::P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat eta, 
		AurynFloat kappa, 
...
P10ConnectionP::P10ConnectionP(SpikingGroup * source, NeuronGroup * destination, 
		AurynWeight weight,std::string filename, AurynFloat sparseness, 
		AurynFloat eta, 
...
: DuplexConnection(source, 
		...
		name)
{
    u_x_file.open(filename.c_str());
	init( eta, kappa, maxweight);
	init_shortcuts();
}

P10ConnectionP::~P10ConnectionP()
{
...
inline AurynWeight P10ConnectionP::dw_pre(const NeuronID post, const AurynWeight * w)
{
	AurynDouble dw = A2_minus*(tr_post->get(post))-delta;
	return dw;
}

inline AurynWeight P10ConnectionP::dw_post(const NeuronID pre, const NeuronID post, const AurynWeight * w, const AurynWeight w0)
{
...
void P10ConnectionP::propagate_forward()
{
	// loop over spikes
...
void P10ConnectionP::propagate_backward()
{
...
void P10ConnectionP::consolidate() {
	for (AurynLong i = 0 ; i < w_solid_matrix->get_nonzero() ; ++i ) {
		AurynWeight * fragile = w->get_data_begin()+i;
		AurynWeight * solid   = w_solid_matrix->get_data_begin()+i;
		*solid = *fragile;
...
void P10ConnectionP::push_attributes()
{
...
void P10ConnectionP::set_tau_f(AurynFloat tauf) {
	tau_f = tauf;
}

void P10ConnectionP::set_tau_cons(AurynFloat taucons) {
...
void P10ConnectionP::set_tau_d(AurynFloat taud) {
	tau_d = taud;
}

void P10ConnectionP::set_ujump(AurynFloat r) {
	Ujump = r;
}

void P10ConnectionP::set_urest(AurynFloat r) {
	Urest = r;
}

void P10ConnectionP::set_weight_a(AurynFloat w0) {
	weight_a = w0;
	weight_b = (weight_a+weight_c)/2;
}

void P10ConnectionP::set_weight_c(AurynFloat w2) {
	weight_c = w2;
	weight_b = (weight_a+weight_c)/2;
}

void P10ConnectionP::set_beta(AurynFloat b)
{
	beta = b;
	beta_fudge = beta; 
...
bool P10ConnectionP::write_to_file(string filename)
{
...
bool P10ConnectionP::load_from_file(string filename)
{
...
void P10ConnectionP::load_fragile_matrix(string filename)
{
...
void P10ConnectionP::randomize_consolidation_variables(AurynFloat mean, AurynFloat std)
{
