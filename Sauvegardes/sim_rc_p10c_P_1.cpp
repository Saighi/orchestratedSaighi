...
int main(int ac, char* av[]) 
{
	...
	//FileInputGroup *stimgroup = new FileInputGroup(size,pat,true,0.1);


	


	sprintf(strbuf, "%s/%s", dir.c_str(), "one_pattern");
	string pat = string(strbuf);
	std::cout << "file:  " << pat << std::endl;
	sprintf(strbuf, "%s/%s", dir.c_str(), "pattern_time");
	string times_pat = string(strbuf);
	std::cout << "file:  " << times_pat << std::endl;
	PoissonFileInputGroupRandom *stimgroup = new PoissonFileInputGroupRandom(size, pat,times_pat, bgrate);
	...
	//sprintf(strbuf, "%s/%s.%d.spikes_ext.txt", dir.c_str(), file_prefix.c_str(), sys->mpi_rank() );
	//SpikeMonitor * mon_ext = new SpikeMonitor( stimgroup, string(strbuf), size );
